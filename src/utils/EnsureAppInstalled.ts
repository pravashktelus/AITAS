#!/usr/bin/env ts-node
/**
 * EnsureAppInstalled.ts — Checks if the APK is installed on the emulator.
 * If not installed, installs it automatically before running tests.
 *
 * Usage: npx ts-node src/utils/EnsureAppInstalled.ts
 */
...
const APK_PATH: string = path.resolve('testdata/appFiles/demo.apk');
...
const devices: string = run('adb devices');
const deviceLines: string[] = devices.split('\n').filter((l) => l.includes('\tdevice'));
...
const packages: string = run(`adb shell pm list packages ${APP_PACKAGE}`);
const isInstalled: boolean = packages.includes(APP_PACKAGE);
...
if (isInstalled) {
  ...
  const result: string = run(`adb install -r "${APK_PATH}"`);
